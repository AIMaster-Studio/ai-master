 ...
 *     故部署路径必须走本文件。
 *
 * 作用与 `functions/api/[[path]].js` 完全等价：把 `/api/*` **字节级透传**到 Node 后端。
 *
 ...
 *   故后端留在 Node 侧，这里只做**字节级透传**的代理。
 *
 * 重要局限（不得包装成"降级仍可用"，见 R5）：
 *   本 worker **不是独立副本**。上游断了这里就是 502。
 *
 * 安全：
 *   - origin 优先从 `API_ORIGIN` 读，旧部署可回退到 `TUNNEL_ORIGIN`；**不硬编码** URL；
...
export default {
  async fetch(request, env) {
    ...
    const origin = String(env.API_ORIGIN || env.TUNNEL_ORIGIN || '').trim();
    if (!origin) {
      return json(500, '服务端未配置上游地址（缺少环境变量 API_ORIGIN / TUNNEL_ORIGIN）。这是部署配置问题，不是临时故障。');
    }
    ...
    try {
      ...
      return json(500, '上游环境变量不是合法 URL。');
    }
    ...
    try {
      ...
      return json(502, '上游学习服务当前不可达，请稍后重试。技术细节：' + detail);
    }
