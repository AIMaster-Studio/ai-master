...
function createMemoryRoutes(options) {
  ...
  return async function handleMemory({ req, url, route, body, send, fail, user }) {
    ...
    if (!isPost) {
      if (action === 'inspect') return send({ memory: await memory.inspect(), graph: await memory.graph() });
      if (action === 'graph') return send({ graph: await memory.graph() });
      if (action === 'l1') {
        const surface = url.searchParams.get('surface');
        if (!surface) fail(400, '缺少 surface。');
        const date = url.searchParams.get('date') || undefined;
        const limit = Number(url.searchParams.get('limit')) || undefined;
        return send({ surface, events: await memory.l1(surface, { date, limit }) });
      }
      if (action === 'l2') {
        const surface = url.searchParams.get('surface');
        if (!surface) fail(400, '缺少 surface。');
        return send({ surface, markdown: await memory.readL2(surface) });
      }
      if (action === 'l3') {
        const slot = url.searchParams.get('slot');
        if (!slot) fail(400, '缺少 slot。');
        return send({ slot, markdown: await memory.readL3(slot) });
      }
      if (action === 'surfaces') {
    ...
    if (action === 'refresh') {
      ...
      if (!surface) fail(400, '缺少 surface。');
      return send({ refreshed: await memory.refreshL2(surface) });
    }
    if (action === 'synthesize') {
      const result = await memory.synthesize();
    ...
    if (action === 'preference') {
      ...
      return send({ preferences: await memory.writePreference(body.text, { operation: body.operation }) });
    }
    if (action === 'clear') {
      // 清空记忆会丢掉全部学习轨迹，属管理操作：与配置写入同一道门。
      requireAdmin(req);
      return send(await memory.clear(body.surface || null));
    }
    fail(404, '接口不存在。');
  };
}
