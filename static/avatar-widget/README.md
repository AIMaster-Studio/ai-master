...
- 🐋 **浮动形象**：默认小鲸鱼，可拖拽，四边四分之一吸附，左吸附时镜像翻转，按压 Q 弹
- 🎨 **形象选择**：预设形象（鲸鱼娘 / 鲸鱼整图 / rua 动图 / 橘猫 / 星宝 / 小机器人）+ 上传自定义形象（PNG/JPG/GIF，自动压缩到 480px，最多保存 4 个）
- 📖 **学习状态监控**：自动识别当前页面（章节 / 学习中心 / 知识星图 / 实验场 / 仪表盘等），统计今日学习时长与各页面访问次数
- ⏱️ **屏幕使用时间**：统计今日屏幕使用总时长（仅页面可见时累计），每天自动重置
- 💬 **气泡台词**：点击鲸鱼随机切换台词（含学习统计 / 卖萌吐槽 / rua 动图），5 秒自动收起
- ⚙️ **汉堡菜单**（悬停鲸鱼右上角 ⚙️ 出现）：形象选择、大小调节（0.6–2.5 倍）、音效开关与音量、气泡开关、今日数据、重置位置 / 清零统计
...
页面是否计入「学习时间」由 `avatar-widget.js` 中 `isStudyPage()` 与 `studyLabel()` 控制（基于 URL 路径匹配），可按需增删。
